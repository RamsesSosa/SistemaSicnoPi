...
const ConsultarClientes = () => {
  ...
  const fetchEquiposCliente = async (clienteId) => {
    try {
      const response = await fetch(`http://127.0.0.1:8000/api/equipos/?cliente=${clienteId}`);
      if (!response.ok) throw new Error('Error al obtener los equipos del cliente');
      const data = await response.json();
          ...
          <table className="elegant-table">
            <thead>
              <tr>
                ...
                <th className="th-date">Fecha Registro</th>
              </tr>
            </thead>
            <tbody>
              {clientesFiltrados.length > 0 ? (
                clientesFiltrados.map((cliente) => (
                  <>
                    <tr 
                      key={cliente.id} 
                      className={`elegant-row ${clienteExpandido === cliente.id ? 'active' : ''}`}
                      onClick={() => handleClienteClick(cliente.id)}
                    >
                      <td className="td-client">
                        <div className="client-info">
                          <span className="client-name">{cliente.nombre_cliente}</span>
                          <span className={`expand-icon ${clienteExpandido === cliente.id ? 'expanded' : ''}`}>
                                ...
                                {equiposCliente.map((equipo) => (
                                  <div key={equipo.id} className="equipo-card">
                                    <div className="equipo-header">
                                      <h5 className="equipo-name">{equipo.nombre_equipo}</h5>
                                      <span className="equipo-brand">{equipo.marca} • {equipo.modelo}</span>
                                    </div>
                                    <div className="equipo-details">
                                      <div className="detail-item">
                                        <span className="detail-label">N° Serie:</span>
                                        <span className="detail-value">{equipo.numero_serie || 'N/A'}</span>
                                      </div>
                                      <div className="detail-item">
                                        <span className="detail-label">Fecha Entrada:</span>
                                        <span className="detail-value">
                                          {equipo.fecha_entrada ? new Date(equipo.fecha_entrada).toLocaleDateString() : 'No registrada'}
                                        </span>
                                      </div>
                                    </div>
                                  </div>
